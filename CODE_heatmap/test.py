...
def parse_gaze_csv(csv_file):
    """
    Parse the gaze data CSV file and extract the first norm_pos x and y for each frame.
    ...
    with open(csv_file, 'r') as file:
        ...
        for row in csv_reader:
            ...
            if world_index != current_world_index:
                # Add previous pos x and y to the list if not None
    ...
    # Add the last frame's pos x and y to the list if not None
    if current_pos_x is not None and current_pos_y is not None:
        gaze_data.append((current_pos_x, current_pos_y))
...
while ret:
    ...
    if common_tags_static and common_tags_frame:
        ...
        # Map the center coordinates of the video frame to global coordinates of the static image
        frame_center = (frame_width // 2, frame_height // 2)
        mapped_x, mapped_y = map_coordinates(frame_center, transformation_matrix)

        # Draw a point in the static image corresponding to the mapped global coordinates
        cv2.circle(static_image_tmp, (int(mapped_x), int(mapped_y)), 5, (255, 0, 0), -1)
    else:
